...
public class JDayChooser extends JPanel implements ActionListener, KeyListener,
	...
	protected String[] dayNames;
        ...
        private ArrayList<Integer> specialDay1 = new ArrayList<>();
        
        private Color specialDay1Color = Color.RED;
        
        private ArrayList<Integer> specialDay2 = new ArrayList<>();
        
        private Color specialDay2Color = Color.GREEN;
...
//            dayChooser.setDayBordersVisible(false);
	...
	public JDayChooser(boolean weekOfYearVisible) {
		...
		sundayForeground = CalendarColors.getTEXT_COLOR();
		weekdayForeground = CalendarColors.getTEXT_COLOR();

		// decorationBackgroundColor = new Color(194, 211, 252);
		// decorationBackgroundColor = new Color(206, 219, 246);
		decorationBackgroundColor = CalendarColors.getACCENT_COLOR();
                                    ...
                                    days[index] = new JButton("x") {
                                            ...
                                            public void paint(Graphics g){
                                                ...
                                                if (doSpecial) {
                                                    ...
                                                    int firstDay = index - tmpCalendar.get(Calendar.DAY_OF_WEEK) - firstDayOfWeek - 4;
                                                    
                                                    if (specialDay1.contains(firstDay)) {
                                                        if (selectedDay == this) g2D.setColor(specialDay1Color.darker().darker());
                                                        else if (getModel().isRollover()) g2D.setColor(specialDay1Color.darker());
                                                        else g2D.setColor(specialDay1Color);
                                                    }
                                                    else if (specialDay2.contains(firstDay)) {
                                                        if (selectedDay == this) g2D.setColor(specialDay2Color.darker().darker());
                                                        else if (getModel().isRollover()) g2D.setColor(specialDay2Color.darker());
                                                        else g2D.setColor(specialDay2Color);
                                                    }
                                                    else {
                                                        if (selectedDay == this) g2D.setColor(selectedColor);
                                                        else if (getModel().isRollover()) g2D.setColor(hoveredColor);
                                                        else g2D.setColor(oldDayBackgroundColor);
                                                    }
                                                }
                                                else {
                                                    if (selectedDay == this) g2D.setColor(selectedColor);
		...
		weekPanel.setLayout(new GridLayout(7, 1));
                weekPanel.setBackground(decorationBackgroundColor);
		weeks = new JButton[7];
		...
		updateUI();
                ...
                this.setForeground(CalendarColors.getTEXT_COLOR());
                this.setFont(new Font("Roboto",Font.BOLD,12));
                hoveredColor = CalendarColors.getHOVER_COLOR();
                decorationBordersVisible = false;
                dayBordersVisible = true;
                dayPanel.setBackground(CalendarColors.getMAIN_COLOR());
	}
        
        private boolean has(int[] arr, int num){
            for (int x : arr) {
                if (x == num) {
                    return true;
                }
	 ...
	 */
	protected void init() {
		oldDayBackgroundColor = CalendarColors.getMAIN_COLOR();
		selectedColor = CalendarColors.getPRESSED_COLOR();
	...
	class DecoratorButton extends JButton {
		...
		public DecoratorButton() {
			setBackground(getDecorationBackgroundColor());
		...
		public void paint(Graphics g) {
			if ("Windows".equals(UIManager.getLookAndFeel().getID())) {
				...
				if (decorationBackgroundVisible) {
					g.setColor(getDecorationBackgroundColor());
				} else {
    ...
    public Color getOldDayBackgroundColor() {
        return oldDayBackgroundColor;
    }

    public void setBackgroundColor(Color backgroundColor) {
        this.oldDayBackgroundColor = backgroundColor;
    }
    ...
    public ArrayList<Integer> getSpecialDay1() {
        return specialDay1;
    }

    public void setSpecialDay1(ArrayList<Integer> specialDay1) {
        this.specialDay1 = specialDay1;
    }

    public Color getSpecialDay1Color() {
        return specialDay1Color;
    }

    public void setSpecialDay1Color(Color specialDay1Color) {
        this.specialDay1Color = specialDay1Color;
    }

    public ArrayList<Integer> getSpecialDay2() {
        return specialDay2;
    }

    public void setSpecialDay2(ArrayList<Integer> specialDay2) {
        this.specialDay2 = specialDay2;
    }

    public Color getSpecialDay2Color() {
        return specialDay2Color;
    }

    public void setSpecialDay2Color(Color specialDay2Color) {
        this.specialDay2Color = specialDay2Color;
    }
